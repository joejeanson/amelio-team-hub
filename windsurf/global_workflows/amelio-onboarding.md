...
> **Note**: This step uses the modular `mongodb-db-manager.sh` script from `amelio-team-hub`. It dynamically detects existing databases and allows creating new ones.

### 6a — MongoDB Database Selection

**Source the database manager module:**
```bash
DB_MANAGER="${TEAM_DIR}/scripts/🔧 Config/mongodb-db-manager.sh"
if [ -f "$DB_MANAGER" ]; then
  source "$DB_MANAGER"
  echo "✓ MongoDB Database Manager loaded"
else
  echo "⚠️ Database manager not found at $DB_MANAGER"
  echo "   Continuing with default 'Freemium' database..."
fi
```
...
# Test connection
if mongosh "mongodb://${MONGO_USER}:${MONGO_PASS}@${MONGO_HOST}:27017/admin?authSource=admin" --quiet --eval "db.adminCommand('ping')" > /dev/null 2>&1; then
  echo "✓ MongoDB connection OK (${MONGO_HOST})"
  
  # List existing databases
  echo ""
  echo "Existing databases:"
  mongosh "mongodb://${MONGO_USER}:${MONGO_PASS}@${MONGO_HOST}:27017/admin?authSource=admin" --quiet --eval "
    db.adminCommand('listDatabases').databases
      .filter(d => !['admin','local','config'].includes(d.name))
      .forEach(d => print('  - ' + d.name))
  " 2>/dev/null || echo "  (none found)"
else
  echo "⚠️ Cannot connect to MongoDB at ${MONGO_HOST}"
  echo "   Make sure MongoDB container is running (Step 5a)"
fi
```

**Ask the user for database strategy:**

> MongoDB Database Setup
> 
> Choose your database approach:

- **A**: Use existing database (select from list)
- **B**: Create a new database with custom name
- **C**: Import the Freemium dump (creates 'Freemium' database)
- **D**: Skip database setup — configure later manually

**If A — Select existing database:**

Ask the user to choose from the list displayed above, or enter a database name:

```bash
echo -n "Enter database name to use: "
read -r SELECTED_DB
echo "Using database: ${SELECTED_DB}"
export MONGO_FREEMIUM_DB="${SELECTED_DB}"
```

**If B — Create new database:**

```bash
echo -n "Enter new database name: "
read -r NEW_DB_NAME

# Create the database (MongoDB creates on first use)
mongosh "mongodb://${MONGO_USER}:${MONGO_PASS}@${MONGO_HOST}:27017/${NEW_DB_NAME}?authSource=admin" --quiet --eval "
  db.createCollection('_init');
  db._init.drop();
  print('Database ${NEW_DB_NAME} created');
" 2>/dev/null

echo "✓ Created database: ${NEW_DB_NAME}"
export MONGO_FREEMIUM_DB="${NEW_DB_NAME}"
```

**If C — Import Freemium dump (classic onboarding):**

Continue to Step 6b below.

**If D — Skip:**

> Skipping database setup. You can configure this later by:
> 1. Running: `yarn mongodb 🔧 – 🍃` in the Config folder
> 2. Or manually editing `appsettings.json` files

Export the selected database name for subsequent steps:
```bash
echo "export MONGO_FREEMIUM_DB='${MONGO_FREEMIUM_DB:-Freemium}'" >> "${TEAM_DIR}/.env"
```

---

### 6b — Import MongoDB Freemium database (Optional)

> **Only run this if you selected option C in Step 6a.**
...
Verify:
```bash
mongosh "mongodb://ameliodb:ameliodb@localhost:27017/${MONGO_FREEMIUM_DB:-Freemium}?authSource=admin" --eval "db.getCollectionNames().length + ' collections imported'"
```
Expected: `121 collections imported` (or similar non-zero number).

### 6c — PostgreSQL migrations (Performance Backend)
